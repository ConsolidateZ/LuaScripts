...
local function RacialPickerSelect(event, player, object, sender, intid, code, menuid)
	...
	if(intid == 8502) then
		...
		object:Announce("You can now relog to faction change",0,player,true)
	end
		if(intid == 8503) then
		player:SetAtLoginFlag(128)
		object:Announce("You can now relog to race change",0,player,true)
	end
	if(intid == 11) then
...
RegisterCreatureGossipEvent(NpcRacialPicker, 1, RacialPicker)
RegisterCreatureGossipEvent(NpcRacialPicker, 2, RacialPickerSelect)

local function OnFristLogin(event, player)
...
RegisterPlayerEvent(30, OnFristLogin)
